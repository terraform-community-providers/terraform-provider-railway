...
`redeploy_environment_ids` only scopes which service instances are redeployed after updates. If omitted, the provider preserves the existing behavior and redeploys all current service instances for the service.
...
- `redeploy_environment_ids` (Set of String) Environment IDs to redeploy after service updates. If omitted, all current service instances are redeployed.
